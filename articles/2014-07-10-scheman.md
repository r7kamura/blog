...
古いスキーマにのみ存在するテーブルがあった場合はDROP TABLEを、
各テーブル内のカラムまたはインデックスの定義に差異が認められた場合にはALTER TABLEを発行する。

Diffオブジェクトの中ではCREATE TABLEやDROP TABLEなどを1命令ごとにHashで表現し、
更に前述したParslet::Transformを利用して、typeで指定された種類のSQL表現に変換する。

## SQL生成
あとで書く: DiffオブジェクトからSQLを生成する
